...
# variables
NAME=engine-YaneuraOu-material1
DIRNAME=YaneuraOu-material1
URL=https://github.com/yaneurao/YaneuraOu.git
COMMIT_HASH=bfd497f683a6d0831bb3809fe5b9876b91b96d23     #Jan 26, 2021 (V6.02)
...
echo -n "外部思考エンジン (やねうら王MATERIAL level1) をインストールしています(少し時間がかかります) ... "
...
if [ ! -d $NAME ]; then
    ...
    for ARCH in `echo $ARCHLIST | sed -e 's/,/ /g'`; do
        ...
        (make -j1 normal TARGET_CPU=`echo $ARCH | tr '[a-z]' '[A-Z]'` COMPILER=${COMPILER} YANEURAOU_EDITION=${EDITION} MATERIAL_LEVEL=${MATERIAL_LEVEL} 2>&1) >> ${LOGDIR}/${NAME}.install.log
